	...
	public void Save(ArrayList <MapNode> mapNodeList) 
	{
		...
		try // try save to specified file
		{
			...
			for (fileLoop=0;fileLoop<mapNodeList.size();fileLoop++)
			{
				...
				if (mapNodeList.get(fileLoop).getAlienList().size() > 0)
				{
					text=text+",A,"+mapNodeList.get(fileLoop).getAlienList().size();
					text+=getAlienNodeSave(mapNodeList.get(fileLoop).getAlienList());
				}
				if (mapNodeList.get(fileLoop).getHeroList().size() > 0)
				{
					System.out.format("test hero\n");
					text=text+",H,"+mapNodeList.get(fileLoop).getHeroList().size();
					text+=getHeroNodeSave(mapNodeList.get(fileLoop).getHeroList());
				}
				if (mapNodeList.get(fileLoop).getGhostList().size() > 0)
				{
					text=text+",G,"+mapNodeList.get(fileLoop).getGhostList().size();
					text+=getGhostNodeSave(mapNodeList.get(fileLoop).getGhostList());
				}
